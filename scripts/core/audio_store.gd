...
class_name AudioStore
extends BoardDataStore

signal audio_added(entity_id: int)
signal audio_changed(entity_id: int)
signal audio_removed(entity_id: int)
signal cleared

const STORE_ID: StringName = &"audios"
...
func set_duration(entity_id: int, duration_seconds: float) -> bool:
	var index: int = get_index(entity_id)
	if index < 0:
		return false
	var safe_msec: int = maxi(0, int(round(duration_seconds * 1000.0)))
	if int(duration_msec[index]) == safe_msec:
		return true
	duration_msec[index] = safe_msec
	_touch(index, entity_id)
	return true
...
func restore_record(record: Dictionary) -> bool:
	...
	return add_audio(entity_id, asset_id, duration_seconds, flags, str(record.get("instance_title", "")))
