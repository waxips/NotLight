...
class_name VideoContextToolbar
extends PanelContainer

signal rename_requested
signal duplicate_requested
signal optimize_requested
...
func configure(display_name: String, available: bool, can_optimize: bool) -> void:
	if _name_label == null:
		return
	_name_label.text = display_name if not display_name.is_empty() else NotLightL10n.text("asset.kind.video")
	_name_label.tooltip_text = _name_label.text if available else NotLightL10n.text("runtime.ui.video_player_overlay.d8c705b0ae")
	...
	if _optimize_button != null:
		_optimize_button.disabled = not available or not can_optimize
		_optimize_button.tooltip_text = (
			NotLightL10n.text("runtime.ui.video_context_toolbar.4bec36521d")
			if available and can_optimize
			else NotLightL10n.text("runtime.ui.video_context_toolbar.dba8fca3e4")
		)
...
func _build_ui() -> void:
	...
	type_label.text = "▶"
	NotLightL10n.bind_tooltip(type_label, "asset.kind.video")
	...
	NotLightL10n.bind_text(_name_label, "asset.kind.video")
	_name_label.theme_type_variation = "CaptionStrongLabel"
	_name_label.custom_minimum_size = Vector2(146.0, 36.0)
	...
	_optimize_button.theme_type_variation = "GhostButton"
	_optimize_button.custom_minimum_size = Vector2(118.0, 36.0)
	_optimize_button.pressed.connect(func() -> void: optimize_requested.emit())
	row.add_child(_optimize_button)

	var duplicate_button: Button = Button.new()
	NotLightL10n.bind_text(duplicate_button, "audio.toolbar.duplicate")
	NotLightL10n.bind_tooltip(duplicate_button, "runtime.ui.video_context_toolbar.8574dd4f5b")
